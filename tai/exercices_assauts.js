...
function displayAssaut(a, container) {
  const derouleHTML = a.deroule
    .map(e => `
      <div class="deroule-item">
        <span class="deroule-num">${e.etape}.</span> ${e.texte}
      </div>
    `)
    .join('');
    ...
    <div class="assaut-display">
      ...
      <div class="assaut-columns">
        <div class="assaut-section">
          <h5>🔑 Points clés</h5>
          <ul>
            ${a.points_cles.map(item => `<li>${item}</li>`).join('')}
          </ul>
        </div>
        ...
        <div class="assaut-section">
          ...
          <ul>
            ${a.erreurs_a_eviter.map(item => `<li>${item}</li>`).join('')}
          </ul>
        </div>
...
async function playAssaut() {
  ...
  await speak(a.assaut);
  await wait(800);

  await speak(`Configuration : ${a.configuration}`);
  await speak(`Objectif : ${a.objectif}`);

  await speak("Voici les points clés");
  for (const p of a.points_cles) await speak(p);

  await speak("Voici les erreurs à éviter");
  for (const e of a.erreurs_a_eviter) await speak(e);

  await speak("Commençons le travail");
...
function exportAssautAsPdf() {
  ...
  win.document.write(`
    ...
    <body>
      <h1>${a.assaut}</h1>
      <p><strong>${a.configuration}</strong></p>
      <p><em>${a.objectif}</em></p>
